...
AllFMs_ls=$(cat "$Subdir"/AllFMs.txt)
...
# ---------------------------------------------------------------------------------------
#AllFMs_ls=`cat "$Subdir"/AllFMs.txt`
for ThisFM in $AllFMs_ls
do

    echo $ThisFM

    # copy over field map pair to workspace 
    cp -r "$Subdir"/func/unprocessed/field_maps/FM_rads_"$ThisFM".nii.gz "$WDIR"/FM_mag_"$ThisFM".nii.gz
    ...
    # nVols=`fslnvols "$4"/FM_mag_"$5".nii.gz`
...
# 	# copy over field map pair to workspace 
# 	cp "$2"/func/unprocessed/field_maps/FM_rads_"$5".nii.gz "$4"/FM_mag_"$5".nii.gz
...
# FOLLOWING 2 LINES AREN'T WORKING
...
# This section is repeated in post_func_preproc_fm.sh ----------------------------------------------------------------------
