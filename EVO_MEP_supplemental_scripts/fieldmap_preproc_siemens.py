...
datadir = '/Users/holland_brown_ra/Desktop/NKI_tmp'
...
for sub in subs:
    ...
    for mag_img in magnitude_imgs:
        ...
        fn_list = fn.split('_')
        filename_new = f"{fn_list[0]}_mag_brain_{fn_list[2]}_{fn_list[3]}_{fn_list[4]}"
...
for sub in subs:
    fm_dir = f'{datadir}/{sub}/func/unprocessed/field_maps'
    bet_magnitude_img = f'{fm_dir}/FM_mag_brain_S{session_num}_R{run_num}_E1.nii.gz' # /path/to/BetMagnitudeNIFTI
    phase_img = f'{fm_dir}/FM_phase_S{session_num}_R{run_num}_E2.nii.gz'
