...
SubjectListText=/Users/holland_brown_ra/Desktop/UW_subjectlist.txt # space delimited list of subject IDs (format like W001, W002, etc.)
Sessions=$(cat "$StudyFolder"/Sessions.txt)
TE=2.399 # Difference in echo times; in EVO study, UW TE = 2.399, NKI TE = 2.46


Subjects=$(cat "$SubjectListText")

for sub in $Subjects; do

    echo -e "$sub"
    FieldMapFolder="$StudyFolder"/"$sub"/func/unprocessed/field_maps

    for s in $Sessions; do

        cp "$FieldMapFolder"/intermediate/S"$s"_AxialField_Mapping_e1_real.nii.gz "$FieldMapFolder"
        cp "$FieldMapFolder"/intermediate/S"$s"_AxialField_Mapping_e2_real.nii.gz "$FieldMapFolder"
        cp "$FieldMapFolder"/intermediate/S"$s"_AxialField_Mapping_e1_imaginary.nii.gz "$FieldMapFolder"
        cp "$FieldMapFolder"/intermediate/S"$s"_AxialField_Mapping_e2_imaginary.nii.gz "$FieldMapFolder"
