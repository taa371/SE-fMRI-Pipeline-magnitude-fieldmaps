...
tmp_dir = [Subdir '/func/task/floop/rois/tmp']; % define the roi directory
...
system(['wb_command -volume-label-import ' tmp_dir '/FinalLabels.nii.gz ' SubcorticalLabels ' ' Subdir '/func/task/floop/rois/Subcortical_ROIs_acpc.nii.gz -discard-others']);
%system(['rm -rf ' tmp_dir]);

% MNInonlinear
tmp_dir = [Subdir 'func/task/floop/rois/tmp_nonlin']; % define the roi directory
%system(['mkdir ' tmp_dir]); % make the temporary dir. 
cd(tmp_dir); % change dir. 
...
system(['fslmerge -t ' tmp_dir '/FinalLabels.nii.gz ' tmp_dir '/Label*_Final.nii.gz']);
system(['fslmaths ' tmp_dir '/FinalLabels.nii.gz -Tmax ' tmp_dir '/FinalLabels.nii.gz']);
system(['wb_command -volume-label-import ' tmp_dir '/FinalLabels.nii.gz ' SubcorticalLabels ' ' Subdir 'func/task/floop/rois/Subcortical_ROIs_nonlin.nii.gz -discard-others']);
% system(['rm -rf ' tmp_dir]);