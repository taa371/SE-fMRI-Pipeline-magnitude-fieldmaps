function c = ft_read_cifti_EVO(filename, varargin)
    ...
    if readdata
      ...
      if isfield(Cifti, 'mapname') && length(Cifti.mapname)>1
        ...
        source.data = dat; % the name of the data will be based on the filename
      end
    end % if data
    ...
    if isfield(Cifti, 'freq')
        source.freq = Cifti.freq;
    end
    if isfield(Cifti, 'mapname')
        source.mapname = Cifti.mapname;
    end
    ...
    % HRB, 2023-09-19: 'copyfields' not recognized & assigning fields to source dynamically didn't work -> try one field at a time
    source.brainstructure = brainordinate.brainstructure;
    source.brainstructurelabel = brainordinate.brainstructurelabel;
    source.pos = brainordinate.pos
